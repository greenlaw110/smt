...
#define smtMachineGetEventCount(machine)    \
    (((smt_stateMachine_ptr_t)machine)->eventCount)
#define smtMachineFindTrigger(machine,event) \
	(&(((smt_stateMachine_ptr_t)machine)->triggerTable[event]))
...
#define smtMachineGetActiveState(machine) \
	(((smt_stateMachine_t *)machine)->activeState)
#define smtMachineSetActiveState(machine, state) \
	(((smt_stateMachine_t *)machine)->activeState = state)
#define smtMachineGetEntryState(machine)    \
    (((smt_stateMachine_ptr_t)machine)->entryState)
